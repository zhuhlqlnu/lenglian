...
function GRulerControl() {
  ...
  me.RESET_BUTTON_IMAGE_ = '/api/map/image/ruler_clear.png';
  me.ENABLE_BUTTON_IMAGE_ = '/api/map/image/ruler_enabled.png';
  me.DISABLE_BUTTON_IMAGE_ = '/api/map/image/ruler_disabled.png';
  me.BACKGROUND_IMAGE_ = '/api/map/image/ruler_background.png'
 ...
 * 返回控件是否已启用
 */
GRulerControl.prototype.isDisabled = function() {
 ...
 * 设置控件的“启用/禁用"状态
 */
GRulerControl.prototype.setDisabled = function(value) {
  var me = this;
  if (value == me.disabled_)
    return;
  me.disabled_ = value;
 
  if (me.disabled_) { 
	  if(me.mapClickHandle_){
...
GRulerControl.prototype.onMapClick_ = function(marker, latlng) {
  ...
  if (newMarker.prev_ != me.head_) {
    ...
    newMarker.segPrev_ = new GPolyline(segment, "#3300CC",3,0.75);
...
GRulerControl.prototype.updateSegments_ = function(marker) {
  ...
  if (marker.segPrev_ && marker.prev_.getLatLng) {
    ...
    marker.segPrev_ = new GPolyline(segment, "#3300CC",3,0.75);
  ...
  if (marker.segNext_ && marker.next_.getLatLng) {
    ...
    marker.segNext_ = new GPolyline(segment, "#3300CC",3,0.75);
...
GRulerControl.prototype.removePoint_ = function(marker) {
  ...
  if (marker.prev_.getLatLng && marker.next_.getLatLng) {
    ...
    var polyline = new GPolyline(segment, "#3300CC",3,0.75);

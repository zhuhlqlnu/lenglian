Ext.onReady(function() {
	...
	sql_array.push("inventory.out.purchase_out_goods_supplier.select");
	sql_array.push('inventory.out.purchase_out_supplier');
	sql_array.push('inventory.out.purchase_out_code');
	sql_array.push('inventory.out.purchase_out_name');
...
//	var goods_code_comp;
	...
	formPanel = new Ext.form.FormPanel({
				...
				items : [purchase_id,order_in_comp, goods_id_comp, company_combo,
						goods_name, goods_unit, price_goods,
...
//================方法提取==============
	...
	function judge_order_in(params_list, params_id_str) {
		if (params_list.length > 0) {
			Zion.db.getJSON("inventory.out.produce_back.query", params_list.shift(),
					function(data) {
...
//								goods_code_comp.setValue(order_in_query[2]);
								...
								goods_unit.setValue(order_in_query[4]);
								price_goods.setValue(order_in_query[5]);
...
//							goods_code_comp.setValue('');
	...
	function update_save(){
		if (formPanel.getForm().isValid() == false&&!same_order_sign) {
			return false;
		} else {
			// 修改操作
			if (win_update) {
				var sm = grid.getSelectionModel().getSelected();
				var id = sm.data.id;
				var params = Ext.zion.form.getParams(formPanel.form, ['goods_id',
						'acount','memo','order_in','id']);
				Ext.zion.db.getJSON("inventory.out.purchase_back.update",
				...
				Ext.zion.db.getJSON("inventory.out.purchase_back.insert",
						params, function(data) {
							if (!data.f) {

Ext.onReady(function() {
	...
	function updateDate(){
		...
		Ext.getCmp("region_name").setValue(sm.data.region_name);
		Ext.getCmp("memo").setValue(sm.data.memo);
		var points = sm.data.points;
		Zion.polygon.edit(map,Zion.polygon.fromString(map,points,'blue',3,0.5,'#ffff00',0.5));
		...
		buttons: [{
            ...
            handler: function(){
				if(add_or_update){
					...
					var memo = Ext.getCmp('memo').getValue();
					var sm = grid.getSelectionModel().getSelected();
					var region_id = sm.data.region_id;
					var points = Zion.polygon.toString(Zion.polygon.endEdit());
					var date = parseInt(new Date().getTime()/1000);
					var params = [region_name,points,date,memo,region_id];
					Zion.db.getJSON('axiom_analyze_region.update',params,function(data){
						if(data.r){

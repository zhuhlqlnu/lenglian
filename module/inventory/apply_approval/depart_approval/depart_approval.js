Ext.onReady(function() {
	...
	var store = new Ext.zion.db.ArrayStore({
				db : {
					alias : "inventory.apply_approval.approval_view_depart"
				},
				root : "r",
...
//================================
	...
	function svaeCheck(form, win) {
		var params_up_apply = [];
		params_up_apply = Ext.zion.form.getParams(form.form, ['approve_part','memo_part','id']);
		var params_lost_apply = [];
		params_lost_apply = Ext.zion.form.getParams(form.form, ['approve_part' , 'apply_id']);
		if(params_up_apply[0]==1){
			Ext.zion.db.getJSON("inventory.apply_approval.out_approval_part_update", params_up_apply, function(
							data) {
						if (!data.f&&data.r>0) {
			...
			Ext.zion.db.getJSON("inventory.apply_approval.out_approval_part_update", params_up_apply, function(
							data) {
						if (!data.f&&data.r>0) {
							Ext.zion.db.getJSON("inventory.apply_approval.lost_out_apply", params_lost_apply, function(
											data) {
										if (!data.f&&data.r>0) {
											Ext.Msg.alert("提示", "提交成功");
											win.close();

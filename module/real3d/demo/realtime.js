...
MTile3D = function() {
	...
	var mTilelayers = [ new GTileLayer(copyCollection, 10, 19) ];
	mTilelayers[0].getTileUrl = function(coord, zoom) {
		 var normalizedCoord = getNormalizedCoord(coord, zoom);
        if (!normalizedCoord) { return null; }
        return "tile/Z" + zoom + "/" + normalizedCoord.y + "/" + normalizedCoord.x + ".png";
	};
	...
	var mMap = new GMapType(mTilelayers, G_NORMAL_MAP.getProjection(), "实景", {
...
MTile3D_pipe = function() {
	var copyCollection = new GCopyrightCollection('MapprSoft');
	var copyright = new GCopyright(1, new GLatLngBounds(new GLatLng(38.7115133718837, 116.107146333474), new GLatLng(38.72896690701, 116.137323523368)), 0, "(C) 2009");
	copyCollection.addCopyright(copyright);
	...
	mTilelayers[0].getTileUrl = function(coord, zoom) {
		 var normalizedCoord = getNormalizedCoord(coord, zoom);
        if (!normalizedCoord) { return null; }
        return "tile_pipe/Z" + zoom + "/" + normalizedCoord.y + "/" + normalizedCoord.x + ".png";
	};
	mTilelayers[0].isPng = function() {
		return true;
	};
	...
	var mMap = new GMapType(mTilelayers, G_NORMAL_MAP.getProjection(), "管道", {
		errorMessage : "",//"很抱歉，在此缩放级别的地图上，未找到该区域。请缩小图像，扩大视野范围。",
		alt : "显示本地地图"
	});
	mMap.getMinimumResolution = function() {
	...
	var map = new GMap2(document.getElementById("map_canvas"), {
		mapTypes : [ new MTile3D(), new MTile3D_pipe()]
	});
	
	
	
	function show_info_window(url, width, height){
		if(!width){
			width = 640;
		}
		...
		title : '详细信息',
		closable : true,
		html: "<iframe scrolling='no' width='" + width +"' height='" + height + "' src='" + url + "'></iframe>",
		buttons : [{
				text : '关闭',
				handler : function() {
					win.close();
	...
	var hot_areas = [
										{
											points:[116.12096635,38.72128173,116.12107977,38.72125198,116.12107977,38.72121479,116.12104444,38.72119062,116.12094032,38.72121665,116.12094032,38.72125570,116.12096077,38.72128173,116.12096635,38.72128173],
											center:[116.121012831991,38.7212333851619],
											info:"<br/>阀门类型:B型<br/>阀门口径:30mm<br/>水表类型:A型<br/>阀门数量:2<br/>水表数量:1<br/>阀门检修时间:2011-12-30<br/>水表检修时间:2011-12-30",
											url:"/module/real3d/demo/info/sbfm.html",
											size:[484,240]											
										},
										{
											points:[116.12040263,38.72058186,116.12620120,38.72430258,116.12622582,38.72427349,116.12043014,38.72053683,116.12040776,38.72057711,116.12040263,38.72058186],
											info:"<b>管段名称:</b>M4<br/><b>敷设形式:</b>埋地<br/><b>埋深（m）:</b>-1.0<br/><b>管线位置:</b>13#楼东南角—21#楼东侧供水干线<br/><b>起点:</b>13#楼东南角<br/><b>终点:</b>21#楼东侧供水干线<br/><b>管线长度（m）:</b>292<br/><b>管材:</b>铸铁管<br/><b>管径（mm）:</b>DN150<br/><b>投产日期:</b>2002.11"
										{
											points:[116.08684793,38.67680468,116.08794000,38.67656896,116.08793105,38.67653166,116.08683310,38.67676576,116.08684793,38.67680468],
										 	info:"编号:DL4-3<br/>电缆型号及规格:YJV22-3*120+1*70<br/>电缆始末端 :四变至43号楼2单元<br/>电缆长度(m）:150<br/>地埋深度（m）:0.5<br/>断路器始端型号及规格:BKM3ZF-250HD34H10<br/>断路器末端型号及规格:SZ-250<br/>断路器数量:3"
										}
									];
										
	function add_hot_area(hot_areas){
		for(var i = 0; i < hot_areas.length; i++){
			(function(hot_area){
				var points = [];
				
				for(var j = 0; j < hot_area.points.length / 2; j++){
					points.push(new GLatLng(hot_area.points[j * 2 + 1], hot_area.points[j * 2]));
				}
				
				var normal = new GPolygon(points, "#f33f00", 1, 0, "#ff0000", 0);
				var focus = new GPolygon(points, "#f33f00", 1, 1, "#ff0000", 0.2);
				var center = null;
				if(hot_area.center){
					center = new GLatLng(hot_area.center[1], hot_area.center[0]);
				}
				
				map.addOverlay(normal);
				
				GEvent.addListener(focus, "click", function(latlng){
						if(hot_area.url){
							show_info_window(hot_area.url, hot_area.size[0], hot_area.size[1]);
						}else{
							if(center){
								map.openInfoWindowHtml(center, hot_area.info);
							}else{
								map.openInfoWindowHtml(latlng, hot_area.info);
							}
						}
				});
				
				GEvent.addListener(normal, "mouseout", function(latlng){
					 focus.remove();
				});
				
				GEvent.addListener(normal, "mouseover", function(latlng){
						map.addOverlay(focus);
				});
		})(hot_areas[i])}
	}
	
	add_hot_area(hot_areas);
	
	//map.overlayMapTypes.insertAt(0, customMapType);
	setDefaultMapUI(map);
	map.setCenter(new GLatLng( 38.6965974210022, 116.103628557379), 13);
	
		var polygon_ = new GPolygon([
	    new GLatLng(38.67678199,116.08683814),
	    new GLatLng(38.67676364,116.08693975),
	    new GLatLng(38.67665780,116.08694540),
	    new GLatLng(38.67655901,116.08691576),
	    new GLatLng(38.67657594,116.08680991),
	    new GLatLng(38.67665921,116.08680003),
	    new GLatLng(38.67678199,116.08683814)    
	  ], "#f33f00", 1, 0, "#ff0000", 0);
    ...
    new GLatLng(38.67678199,116.08683814)    
 	...
 	GEvent.addListener(polygon, "click", function(latlng){
		 Zion.db.getJSON('pipe.management.data.point.id',[8955],function(data){
				...
				var x = 116.086867775482;//data_value[0][4];
				var y = 38.6767029581083;//data_value[0][5];
				...
				Zion.db.getJSON('pipe.data.point.last_data.select',[overlay_id],function(data){
					if(data.r){
						...
						Zion.db.getJSON("axiom_overlay_point.axiom_info_attribute_value.select",[overlay_id,overlay_type_id,overlay_id],function(data){
							...
							//pointHtml += "<iframe src='http://202.8.157.3/module/real3d/demo_browse/pdl.html' width='100' />";

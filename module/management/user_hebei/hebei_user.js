	...
	var id = []; // 编号
		...
		function add_user_Form(formPanel, win) {
			...
			var params = Ext.zion.form.getParams(formPanel, [ 'login_name',
					...
					'address', 'status', 'corp_id', 'enable', 'memo','post_name' ]);
			Ext.zion.db.getJSON("axiom_user.user_id.nextval.select", null, function(data) {	
				if(data.r && !data.f){
					var alias;
					if(params[9] == 1){
						alias = 'hebei_axiom_user.insert';
					}else{
						alias = 'hebei_axiom_user.insert_disable';
					}
		...
		function update_user_Form(formPanel, win) {
			...
			var params = Ext.zion.form.getParams(formPanel, [ 'login_name',
					'password', 'user_name', 'telephone', 'fax', 'email',
					'address', 'status', 'enable', 'memo','post_name', 'user_id', 'corp_id' ]);
			var alias;
			if(params[8] == 1){
				alias = 'hebei_axiom_user.update';
			}else{
				alias = 'hebei_axiom_user.update_disable';
			}
			Ext.zion.db.getJSON(alias, params, function(data) {
				if (data.f) {
					if (data.f.indexOf('PK_USER_LOGIN')) {
			...
			store
					.constructor( {
						...
						fields : [ 'user_id', 'login_name', 'password',
								...
								'reg_date', 'reg_user_id', 'memo','post_name','photo_file' ]
					});
			store.load( {
				params : {
					start : 0,

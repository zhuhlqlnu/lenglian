...
Ext.onReady(function() {
	...
	select_time = function (){
     ...
     var tbar2 = new Ext.Toolbar({
    	 ...
    	 items:[{
			...
			//常规报表
				if(getCheckedTarget() == ""){
					Ext.Msg.alert('提示 ','请选择监控目标');
					return;
				}
				tbar_select=0;
				select_time();
				var alias;
				check_target = getCheckedTarget().toString();
				if(check_target==""){
					params = [new_start_time,new_end_time];
					alias = 'm_sales.sales_manage.daily.report_manage.customer_visits.select';
				}else{
					params = [check_target,check_target,check_target,check_target,check_target,new_start_time,new_end_time];
					alias = 'm_sales.sales_manage.daily.report_manage.customer_visits.target.select';
				}
	...
	var fields= ['point_name','visit_count','point_id','target_id'];
     ...
     var grid = new Ext.grid.GridPanel({
        ...
        // loadMask : {msg:'查询中...'},
         ...
         columns: [
             //sm,
             {header: '标注名称',width: 80,sortable: true, dataIndex: 'point_name'}, 
             {header: '拜访次数',width: 80,sortable: true, dataIndex: 'visit_count'},
             {header: '操作',width: 80,sortable: true,renderer:function(point_id,c,record){
             	return '<a href="javascript:select_target('+ point_id +');">详细信息 </a>';
             }}
         ],
	...
	target_fields  = ['point_name','target_name','arrive_time','leave_time','interval','geocoding']
	target_store = new Ext.zion.db.ArrayStore({
 		db:{
 			params:[time_start,time_end],
 			alias: "m_sales.sales_manage.daily.report_manage.customer_visit.point.select"
 		},
 		root: 'r',
 	...
 	target_grid = new Ext.grid.GridPanel({
        ...
        // loadMask : {msg:'查询中...'},
         ...
         columns: [
             //sm,
             {header: '到访客户',width: 60,sortable: true, dataIndex: 'point_name'}, 
             {header: '拜访业务员',width: 60,sortable: true, dataIndex: 'target_name'}, 
             ...
             {header: '位置描述',width: 250,sortable: true, dataIndex: 'geocoding',renderer : function(geocoding) {
				if (geocoding == null) {
					return '';
				}
				return '<span style="display:table;width:100%;" qtip="'+ geocoding + '">' + geocoding + '</span>';
             }}
         ],
         viewConfig:{
         	autoFill : true,
...
function select_target(point_id){
	var params;
	//自定义报表
	var s = Ext.getCmp('startDate').getRawValue() + ' ' + Ext.getCmp('startTime').getValue() + ':00';
	var std = (new Date(Date.parse(s.replace(/-/g, "/")))).getTime()/1000;
	...
	if(tbar_select==0){
		select_time();
		params = [point_id,check_target,check_target,check_target,check_target,check_target,new_start_time,new_end_time];
	}else{
		params = [point_id,check_target,check_target,check_target,check_target,check_target,std,end];
	}
	
	target_grid.store.constructor({
		db : {
			params : params,
			alias : 'm_sales.sales_manage.daily.report_manage.customer_visit.point.point_id.select'
		},
		root : "r",
		fields : target_fields
	});

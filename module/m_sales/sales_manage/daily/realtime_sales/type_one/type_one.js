	Ext.onReady(function(){
        	 ...
        	 items:[{
				...
				//常规报表
    				...
    				/*grid.store.constructor({
						db : {
							params : params,
							alias : 'sales_goods_record.select'
						},
						root : "r",
						fields : fields
					});
				 ...
				 handler : function(){
				 	...
				 	Ext.Msg
						.alert(
								...
								"<a href='"
										+ Zion.report
												.getURL('sales_goods_record.select',params)
				 ...
				 handler : function(){
					...
					Ext.Msg
						.alert(
								...
								"<a href='"
										+ Zion.report
												.getURL('sales_goods_record.select',params_customize)
										+ "' target='_blank' onclick='Ext.Msg.hide() '>点击此链接下载文件</a>");
	          	 }
			 }]
         });
		...
		var fields= ['goods_id','goods_sn','goods_name','goods_type','user_name','recv_time','goods_quantity','goods_saleroom'];
         ...
         var grid = new Ext.grid.GridPanel({
             ...
             columns: [
                 ...
                 {header: "日期", width: 160, sortable: true,renderer: timeStr, dataIndex: 'recv_time'}, 
		...
		function type_one_search(params_list){
			loadMask.show();
			Zion.db.getJSON("sales_goods_record.select", params_list, function(data) {
				if(data.r){
					store.loadData(data);
					var result = data.r;
					...
					for(var i = 0;i < result.length;i++){
						goods_quantity += result[i][6];
						goods_saleroom += result[i][7];
					}

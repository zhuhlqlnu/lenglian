Ext.onReady(function() {
	...
	var store = new Ext.zion.db.ArrayStore({
		db : {
			alias : "axiom_overlay_point.select"
		},
		root: 'r',
		fields: [
        ...
        tbar: [
            { 
				...
				handler: function(){
					...
					Zion.db.getJSON('axiom_overlay_point.axiom_overlay_attribute_value.update',[overlay_id],function(data){
						if(!data.f){
							if(data.r == ''){
								return;
							}
...
//							attribute_grid.setSource(p_source_update);
						...
						/*	
							 var data = [];
							  data.push(new Ext.grid.PropertyRecord({name: 1, value: 1}, "332r4"));
							  data.push(new Ext.grid.PropertyRecord({name: 1, value: 1}, 2222));
						      attribute_grid.getStore().loadRecords({records: data}, {}, true);
*/
						}
					})
					modify_x = sm.data.x;
    ...
    var type_store = new Ext.data.SimpleStore({
			...
			proxy : new Ext.data.ScriptTagProxy({
				url : ZionSetting.db.url + '/' + Zion.token
						+ '/axiom_overlay_type.point.select'
			})
		});
		...
		listeners : {
			'select' : function(this_, record, index){
	    		...
	    		var attribute_store = new Ext.data.SimpleStore({
					proxy : new Ext.data.ScriptTagProxy({
						url : ZionSetting.db.url + '/' + Zion.token
								+ '/poi_layout.type_attribute.type.search/['+overlay_type_id+']'
					}),
					root : "r", 
    	...
    	if(e.value == '图片'){
    		var formPanel =  new Ext.form.FormPanel( {
				...
				items : [{
					...
					blankText : '请选择上传图片',
					buttonText: '',
					regexText :'支持jpeg,jpg,gif,png的图片上传',
    		...
    		var win  = new Ext.Window({
				...
				buttons : [ {
					...
					handler : function() {
						var sm = grid.getSelectionModel().getSelected();
						var overlay_id = sm.data.overlay_id;
						var attribute_id = attribute_grid.getStore().data.items[0].id;
						...
						var path = attribute_id+overlay_id+fileType;
						var url = '/upload/'+Ext.zion.token+'/attribute/'+Zion.util.encodeParam([path]);
							...
							formPanel.getForm().submit({
								...
								failure : function(form,action){
									if(action.result.r == 'ok'){
										...
										attribute_grid.setSource({number:'/upload/user/'+path});
										e.value = '/upload/user/'+path;
										win.close();
									}else{
						...
						if (data.f) {
							...
							if (data.r == 1) {
								for(var j = 0 ; j < p_source_value.length; j++){
									...
									Zion.db.getJSON('axiom_overlay_attribute_value.insert',attribute_params,function(data){
										if(!data.f){
				...
				if (data.f) {
					...
					if (data.r == 1) {
						for(var i = 0; i < p_source_update_value.length; i ++){
							...
							Zion.db.getJSON('axiom_overlay_attribute_value.update',params_update,function(data){
								if(!data.f){
					...
					var delete_poi = function(id) {
						if (id.length > 0) {
							...
							Ext.zion.db.getJSON("axiom_overlay_point.delete", [overlay_id], function(data) {
								if(data.r){
									Zion.db.getJSON('axiom_overlay_attribute_value.delete',[overlay_id],function(data){
										if(!data.f){
											delete_poi(id);
										}else{

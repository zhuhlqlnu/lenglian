...
Ext.onReady(function() {
	...
	function save_history(win,form){
		var params = [];
		params = Ext.zion.form.getParams(form, [ 'temperature', 'ad1', 'ad2', 'voltage', 'track_id' ]);
			...
			Ext.Msg.confirm('删除确认', '删除的数据将不可恢复，你是否确认删除选中的记录？', function(btn) {
				if (btn == 'yes') {
					for ( var i = 0; i < sm.length; i += 1) {
						...
						if (member) {
							var gps_time = utc_to_timestamp(member.gps_time);
							var target_id = target_name_combo.getValue();
							id.push({t:target_id,g:gps_time});
						} else {
	...
	var deleNext = function() {
		if (id.length > 0) {
			var params = id.pop();
			var target_id = params.t;
			var gps_time = params.g;
			Zion.db.getJSON('pipe.cathodic.history_operation.delete',[target_id,gps_time],function(data){
				if(!data.f){

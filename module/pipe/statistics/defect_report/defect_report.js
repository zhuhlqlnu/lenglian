Ext.onReady(function(){
	...
	var print_content =  "<iframe frameborder='0' id='print' style='width:100%;height:100%;' scrolling='auto' src='print.htm?token="+token+"' ></iframe>";
	/**常规报表时间**/
	...
	fields= ['id','recv_time','gps_time','longitude','latitude','name','login_name','memo','status','pdt_id','dictionary_id'];
		 ...
		 items:[{
			...
			//常规报表
			 ...
			 handler : function(){
				select_time();
				is_custom = false;
				top.is_custom = is_custom;
				top.new_start_time = new_start_time;
				top.new_end_time = new_end_time;
				top.selectCorpId = selectCorpId;
...
//         store.load({params:{start:0,limit:Ext.zion.page.limit,terminal_status:16}});
		...
		var process_grid = new Ext.grid.GridPanel({
			...
			columns : [new Ext.grid.RowNumberer({header:'序号',width:35}),
			{
				...
				renderer : function(v, c, r) {
					...
					var grid_id = sm.data.id;
					if(type == 3){
						return "<a href=# onclick='show_win("
								+ grid_id + ",false)'>" + grid_id + ".jpg</a>";
					}
					if (way == 1) {
						return "";
					} else {
	...
	function show_win(id,is_process){
		var path;
		if(is_process){
			path = '<span><img id="rotImg" width="100%" height="100%" src="/uploaded/visitator/defect_process/'+id+'.jpg"/></span>';
		}else{
